...
def plot_flux_map_network_effect2(
    ds_all: dict[xr.Dataset],
    models1: list[str],
    models2: list[str],
    models1_label: str,
    ...
    ds2 = combine_map_dataset({k: ds_dict[k] for k in models2})
    ds_diff = {}
    ds_diff['models1'] = ds1['combined']
    ds_diff['models2'] = ds2['combined']
    ds_diff['diff'] = make_model_diff_ds(ds1['combined'], ds2['combined'])

    # Resample over the whole time period (season=None)
    for m, ds in ds_diff.items():
        ds_diff[m] = resample_over_period(
            ds, chop_by=None, resample_uncert_correlation=resample_uncert_correlation
        )[0]
    ...
    fig, ax = plt.subplots(n_rows, n_cols, figsize=figsize, layout="compressed")
    for col, (model, ds) in enumerate(ds_diff.items()):
        ax_i = ax[col]
        lon, lat = ds.longitude, ds.latitude
        ...
        # Add titles
        if model == "diff":
            ax_i.set_title(f"{models1_label} - {models2_label}")
        elif model == "models1":
            ax_i.set_title(models1_label)
        else:
            ax_i.set_title(models2_label)
