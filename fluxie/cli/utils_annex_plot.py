...
def create_str_dataframe(
    ...
    for species in data.species.unique():
        ...
        _, exp = np.stack(
            data_per_species.mean_val.apply(
                lambda x: (
                    np.array(f"{x:.2e}".split("e")).astype(float)
                    if f"{x:.2e}" != "0.00e+00"
                    else [np.nan, np.nan]
                )
            ).values
        ...
        if max_exp < -1 and max_exp >= -4:
            ...
            data_per_species["units"] = (
                "\\footnotesize{$\\left (\\rm{GgCO}_{2}\\rm{\\text{-}eq} \\cdot \\rm{yr}^{-1} \\right )$}"
            )
            max_exp += 3
        elif max_exp < -4:
            for var in ["mean_val", "min_unc", "max_unc"]:
    ...
    for species in output.species.unique():
        if species not in species_name.keys():
            species_name[species] = species.replace("hfc", "HFC-").replace(
                "pfc", "PFC-"
            )
    ...
    columns = np.concatenate(
        [
            index_col,
            np.sort([col for col in output.columns if col not in index_col]),
        ]
    )
