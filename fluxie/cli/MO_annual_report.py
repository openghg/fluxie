...
def make_AR_table(
    ...
    logger.warning(
        "If you get an 'Index contains duplicate entries' error, "
    ...
    all_years = [
        t for t in all_sp_res[r][sectors[0]] if t not in ["species", "units", "source"]
    ]
    ...
    caption = (
        "\n \\caption{"
        + f"{species_print} emission {units_print} estimates with 1$\sigma$ uncertainty"
        + "}"
    )
        ...
        for s in sectors:
            ...
            if include_inventory:
                ...
                if include_inventory_uncert:
                    ...
                    type_title_csv += (
                        f",Inventory{sector_name_csv},{inv_model}{sector_name_csv}"
                    )
                region_title += f" & {region_name} & {region_name}"
                fill_line += " & &"

            else:
                type_title += f" & {inv_model_latex}{sector_name}"
                if include_inventory_uncert:
        ...
        for r in regions:
            for s in sectors:
                if r != "UK" and int(t) > 2023:
                    if include_inventory:
                        new_line += f"&  &  {all_sp_res[r][s][t].values[0]}".replace(
                            "\\pm", "${\\pm}$"
                        )
                        new_line_csv += f",,{all_sp_res[r][s][t].values[0]}".replace(
                            " \\pm ", ","
                        )
                    ...
                    if include_inventory:
                        if include_inventory_uncert == True:
                            ...
                            extra_commas = ","
                        new_line += f"&  {all_sp_res[r][s][t].values[1]}&  {all_sp_res[r][s][t].values[0]}".replace(
                            "\\pm", "${\\pm}$"
                        )
                        if all_sp_res[r][s][t].values[1] == " ":
                            new_line_csv += f",{extra_commas}{all_sp_res[r][s][t].values[0]}".replace(
                                " \\pm ", ","
                            )
                        else:
                            new_line_csv += f",{all_sp_res[r][s][t].values[1]},{all_sp_res[r][s][t].values[0]}".replace(
                                " \\pm ", ","
                            )
