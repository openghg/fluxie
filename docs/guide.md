...
Fluxie data is organized in xarray Datasets. Each model is represented by a dataset with a common strucutre. 

In Fluxie each plot is done by a function that takes the dataset and various parameters as input.  
The plots are done using matplotlib. 

For the parameters, it is also possible to define config files with all the parameters for the plots.


## Formatting data for fluxie 

If you are adding a new model to fluxie, you have to produce netcdfs compabile with the fluxie structure. You can find an example of the structure for an intercomparison in the `data/tests` folder. 

All variables are listed in the `data/templates` folder. 
...
## Running fluxie on the ICOS Jupyter Hub
...
## Creating a new plot function

In fluxie, each plot correspond to a function.
First check if there is already a function similar to the one 
you want to create. If yes, it is best to add functionality to the existing function. 

Otherwise here are some guidelines to create a new plot function:
- Start by copying an existing function.
- For arguments, use the same names/data structure as in other functions. 
- Document the function properly. 
- Write a test for the function using the test data already existing (see tests section below).
- Think that the plot should look good with the default parameters. 

## Merging your code to fluxie

fluxie is an open source project. We are happy to receive any PR.
If you add new functionality or fix bugs, we appreciate if you open an issue first to discuss the changes. 

Fluxie maintainers will do their best to review your PR and suggest improvements if needed.

Plots functions are integrated in fluxie core.
Preprocessing scripts 
can be added as example but won't be maintained by the fluxie team.


## Tests

Each function in fluxie requires a unit test.

In general it is good to test the function not only with default parameters but also with special cases. 
